...
import { Heart, Headphones, Check, Play, Pause, RotateCcw, ZoomIn, ZoomOut, BookOpen, ChevronDown } from 'lucide-react';
  ...
  const togglePlay = () => {
    if (isPlaying) {
      ...
      if (pausedRef.current && pausedRef.current.src) {
        ...
        pausedRef.current = null;
        audioRef.current = audio;
        registerRef.current(audio, { label: `${surahName} · v.${startVerse}-${endVerse}`, returnPath: window.location.pathname + window.location.search, surahNumber, startVerse });
      ...
      <div className="text-center space-y-5">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-center gap-2 text-white/50 text-xs">
            <Headphones className="h-3.5 w-3.5" style={{ color: '#d4af37' }} />
            <span>Écoute le récitateur</span>
          </div>
