...
import { Calculator, Calendar, ChevronDown, ChevronUp, Target, Check, Edit3, Lock, X } from 'lucide-react';
...
export function CuriosityCorner({ pagesRead, onCreateGoal }: CuriosityCornerProps) {
  ...
  const [originalGoalType, setOriginalGoalType] = useState<'pages_per_day' | 'duration_days'>('pages_per_day');
  const [originalGoalValue, setOriginalGoalValue] = useState(5);
  ...
  const fetchActiveGoal = useCallback(async () => {
    ...
    try {
      ...
      if (data) {
        ...
        setGoalValue(goal.target_value);
        setOriginalGoalType(goal.goal_type);
        setOriginalGoalValue(goal.target_value);
      }
    } catch (error) {
  ...
  const handleSetGoal = async () => {
    ...
    try {
      ...
      setIsEditing(false);
      toast.success('Objectif enregistré! Bismillah 🌙');
    } catch (error) {
  ...
  const handleEditGoal = () => {
    ...
    if (activeGoal) {
      setOriginalGoalType(activeGoal.goal_type);
      setOriginalGoalValue(activeGoal.target_value);
    }
    setIsEditing(true);
  };
  ...
  const handleCancelEdit = () => {
    ...
    setGoalValue(originalGoalValue);
    setIsEditing(false);
  };
  ...
  const handleValueChange = (value: string) => {
    ...
    if (!isNaN(num) && num >= 1) {
      const max = goalType === 'pages_per_day' ? TOTAL_QURAN_PAGES : 1000;
      setGoalValue(Math.min(num, max));
    } else if (value === '') {
      setGoalValue(1);
    }
  };
      ...
      >
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-xl bg-gold/20 flex items-center justify-center flex-shrink-0">
            <Calculator className="h-6 w-6 text-gold-foreground" />
          </div>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          {activeGoal && (
            <div className="flex items-center gap-1 text-xs bg-primary/10 text-primary px-2 py-1 rounded-full">
              ...
              <span className="hidden xs:inline">Défini</span>
            </div>
          )}
          <motion.div
                ...
                >
                  <div className="flex items-start justify-between gap-3 mb-4">
                    ...
                    <Button
                      ...
                      className="flex-shrink-0 h-11 px-3 rounded-xl border-primary/30 hover:bg-primary/10 touch-target"
                    >
                ...
                >
                  {isEditing && (
                    <div className="flex items-center justify-between bg-muted/30 rounded-xl p-3">
                      <h4 className="font-semibold text-foreground">Mode édition</h4>
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={handleCancelEdit}
                        className="text-muted-foreground hover:text-destructive h-10 px-3 touch-target"
                      >
                  ...
                  <Tabs value={goalType} onValueChange={(v) => setGoalType(v as typeof goalType)}>
                    <TabsList className="w-full h-14 grid grid-cols-2 gap-1 p-1 bg-muted/50 rounded-xl">
                      <TabsTrigger 
                        value="pages_per_day" 
                        className="text-sm sm:text-base font-medium rounded-lg data-[state=active]:bg-white data-[state=active]:shadow-sm h-full touch-target"
                      >
                        Pages/jour
                      </TabsTrigger>
                      <TabsTrigger 
                    ...
                    <TabsContent value="pages_per_day" className="space-y-4 mt-4">
                      <div className="flex items-center justify-center gap-4">
                        <Input
                          ...
                          className="text-center text-3xl font-bold h-16 w-28 rounded-xl border-2 border-sage/30 focus:border-primary bg-white touch-target"
                        />
                        <span className="text-base sm:text-lg text-muted-foreground">pages/jour</span>
                      </div>
                      <div className="bg-muted/50 rounded-xl p-4 text-center">
                        <p className="text-base text-muted-foreground">
                          ≈ <span className="font-bold text-foreground text-xl">{daysToComplete}</span> jours pour terminer
                        </p>
                      </div>
                    </TabsContent>
                    ...
                    <TabsContent value="duration_days" className="space-y-4 mt-4">
                      ...
                      <div className="bg-muted/50 rounded-xl p-4 text-center">
                        <p className="text-base text-muted-foreground">
                          ≈ <span className="font-bold text-foreground text-xl">{pagesPerDay.toFixed(1)}</span> pages/jour
                        </p>
                      </div>
                    </TabsContent>
                  </Tabs>

                  {/* Set Goal Button - Only explicit click saves */}
                  {onCreateGoal && (
                    <Button
                      onClick={handleSetGoal}
                      disabled={isSaving}
                      className="w-full h-14 bg-gold hover:bg-gold/90 text-gold-foreground font-bold text-lg rounded-xl shadow-lg transition-all disabled:opacity-50 touch-target"
                    >
                      {isSaving ? (
                        ...
                        <>
                          ...
                          {isEditing ? 'Mettre à jour' : 'Définir cet objectif'}
                        </>
                      )}
                    </Button>
                  )}
                  ...
                  <p className="text-xs text-center text-muted-foreground">
                    Les modifications ne sont enregistrées qu'après avoir cliqué sur le bouton ci-dessus
                  </p>
                </motion.div>
              )}
