...
export default function SuiviHifz_PageB() {
  const { user } = useAuth();
  ...
  if (loading) {
    return (
      <AppLayout title="Mon Suivi Hifz — Version B">
  ...
  return (
    <AppLayout title="Mon Suivi Hifz — B" hideNav bgClassName="bg-[var(--p-bg)]">
      {/* Header — more spacious without calendar */}
      <div className="flex items-center gap-3 pt-4 pb-6">
        ...
        <span className="ml-auto px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider" style={{ background: 'var(--p-accent)', color: 'var(--p-on-dark)' }}>
          Version B
        </span>
      </div>
      ...
      {/* Floating Calendar Bar — BOTTOM position */}
