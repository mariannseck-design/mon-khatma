...
export default function SuiviHifz_PageA() {
  const { user } = useAuth();
  ...
  const juzData = useMemo<JuzData[]>(() => {
    ...
    return Array.from({ length: 30 }, (_, i) => {
      ...
      let totalVerses = 0;
      for (const s of juzSurahs) totalVerses += s.versesCount;
      ...
      for (const m of juzMemorized) {
        ...
        if (!alreadyReviewedToday && (!earliestReview || m.next_review_date < earliestReview)) earliestReview = m.next_review_date;
        if (m.last_reviewed_at && (!latestReviewed || m.last_reviewed_at > latestReviewed)) latestReviewed = m.last_reviewed_at;
      }
      ...
      const surahs = Array.from(surahDetails.entries()).map(([num, r]) => {
        ...
        return { number: num, name: s?.name || `Surah ${num}`, verseRange: `v.${r.minVerse}-${r.maxVerse}` };
      });
      const isOverdue = earliestReview ? isPast(startOfDay(new Date(earliestReview))) && !isToday(new Date(earliestReview)) : false;

      return { juzNumber: juzNum, totalVerses, memorizedVerses: memorizedVerseCount, percentage, surahs, avgEase, nextReview: earliestReview, lastReviewed: latestReviewed, retentionLabel: getRetentionLabel(avgEase), isOverdue };
    });
  ...
  // Compute pages and juz/hizb stats
  const uniquePages = useMemo(() => {
    const pages = new Set<number>();
    for (const m of memorized) {
      const sp = versePages.get(`${m.id}_start`) || 0;
      const ep = versePages.get(`${m.id}_end`) || 0;
      for (let p = sp; p <= ep; p++) if (p > 0) pages.add(p);
    }
    return pages.size;
  }, [memorized, versePages]);

  const juzStarted = activeJuz.length;
  const hizbCount = juzStarted * 2;

  if (loading) {
    return (
      <AppLayout title="Mon Suivi Hifz — Version A">
  ...
  return (
    <AppLayout title="Mon Suivi Hifz — A" hideNav bgClassName="bg-[var(--p-bg)]">
      {/* Header */}
      <div className="flex items-center gap-3 pt-4 pb-4">
        ...
        <span className="ml-auto px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider" style={{ background: 'var(--p-primary)', color: 'var(--p-on-dark)' }}>
          Version A
        </span>
      </div>

      {/* Calendar Bar — HEADER position */}
      <WeeklyCalendarBar selectedDate={selectedDate} onSelectDate={setSelectedDate} variant="header" />

      {/* 4 Stats Grid */}
      <div className="grid grid-cols-4 gap-2 mb-6">
        {[
          { label: 'Versets', value: totalAyats, emoji: '📝' },
          { label: 'Pages', value: uniquePages, emoji: '📄' },
          { label: 'Juz', value: juzStarted, emoji: '📚' },
          { label: 'Hizb', value: hizbCount, emoji: '🔖' },
        ].map((stat, i) => (
          <div key={i} className="rounded-3xl p-3 text-center" style={{ background: 'var(--p-card)', border: '1px solid var(--p-border)', boxShadow: 'var(--p-card-shadow)' }}>
            <div className="text-lg font-bold" style={{ color: 'var(--p-primary)' }}>{stat.value}</div>
            <div className="text-[9px] uppercase tracking-wider" style={{ color: 'var(--p-text-55)' }}>{stat.label}</div>
          </div>
        ))}
      </div>
      ...
      {visibleJuz.length === 0 && (
        <div className="rounded-3xl p-6 text-center" style={{ background: 'var(--p-card)', border: '1px solid var(--p-border)' }}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 pb-8">
        {visibleJuz.map(juz => {
          ...
          if (!active) {
            return (
              <div key={juz.juzNumber} className="rounded-3xl p-3 flex flex-col items-center gap-1 opacity-50"
          ...
          return (
            <div key={juz.juzNumber}
              className={`rounded-3xl transition-all duration-300 cursor-pointer col-span-2 sm:col-span-3 ${expanded ? 'shadow-lg' : 'hover:shadow-md'}`}
              style={{
            ...
            >
              <div className="p-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  {expanded ? <ChevronUp className="w-4 h-4" style={{ color: 'var(--p-primary)' }} /> : <ChevronDown className="w-4 h-4" style={{ color: 'var(--p-text-55)' }} />}
                </div>
                ...
                <div className="flex items-center gap-3">
                  <div className="relative flex-shrink-0">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <span className="font-bold text-base" style={{ color: 'var(--p-primary)' }}>{juz.percentage}%</span>
                    </div>
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="text-xs mb-1" style={{ color: 'var(--p-text-65)' }}>{juz.memorizedVerses} ayats mémorisées</div>
                    <div className="h-1.5 rounded-full overflow-hidden" style={{ background: 'var(--p-track)' }}>
                      <div className="h-full rounded-full transition-all duration-500" style={{ width: `${juz.percentage}%`, background: 'var(--p-primary)' }} />
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {juz.lastReviewed && (
                        <div className="rounded-lg p-2" style={{ background: 'var(--p-track)' }}>
                          ...
                          <div className="text-xs font-medium" style={{ color: 'var(--p-primary)' }}>{formatSmartDate(juz.lastReviewed)}</div>
                        </div>
                      )}
                      {juz.nextReview && (
                        <button onClick={(e) => { e.stopPropagation(); navigate('/muraja'); }}
      ...
      <div className="flex justify-center pb-24">
        <button onClick={() => setShowAllJuz(!showAllJuz)}
